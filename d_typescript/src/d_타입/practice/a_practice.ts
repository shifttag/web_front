...
function doubleOrNothing(fff: Union){
  if(typeof fff === 'number'){
    fff = fff * 2;
    return fff;
  } else {
    return fff;
...
// - PersonType 타입과 ContactDetails 타입을 결합하여 Employee 타입을 생성
// - Employee 타입은 PersonType의 모든 속성(name, age)과 ContactDetails의 모든 속성(email, phone)을 포함
type PersonType = {
  name: string;
  age: number;
}
...
console.log(newCar);  // { make: 'kia', model: 'k8', engineType: '하이브리드', horsePower: 100 }