...
function addTodo(content) { // 함수 호출 시 인자로 할 일의 내용을 전달받음
  // 새로운 할 일 생성
  const newTodo = {
    id: (todos.length + 1), // 이전 배열의 길이 + 1 === 인덱스 번호 + 1
    content: content,
    completed: false
  };
  todos.push(newTodo);
...
function toggleTodo(id) {
  todos = todos.map((todo) => {
    ...
    if(todo.id === id){
    // 해당 할 일의 완료 상태를 토글 시키고 새로운 객체로 반환
    // >> 스프레드 연산자(...)

    // let arr = [1, 2, 3];
    // let arr2 = [...arr]; >> arr1의 요소만 가져와서 새로운 배열을 생성 (메모리 주소가 다름)
