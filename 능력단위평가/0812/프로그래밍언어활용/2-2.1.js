...
class ShoppingCart {
  ...
  calculateTotal() {
    let total = 0;
    for (let i = 0; i < this.items.length; i++) { /* 조건문 수정 (items의 길이를 크거나 같다일 때 total구할때 items[i]의 마지막 인덱스 번호가 없기때문에 수정) 
      ex) this.items.length 가 2라면 items의 인덱스 번호는 0번과 1번까지 있다 조건을 i <= this.items.length로 두면 i가 2일때도 참이기 때문에 total식에서 this.items[2].price 에서 this.items[2]라는 인덱스가 없기 때문에 오류가 난다 */
